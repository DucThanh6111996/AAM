...
import com.viettel.it.object.CheckCommandObject;
...
//            orders.put("nodeType.typeName", "ASC");
    ...
    public void onSaveOrUpdate() {
        try {
            ...
            if (isEdit && !isClone) {
                ...
                try {
                    LogUtils.logAction(LogUtils.appCode, startTime, new Date(), SessionUtil.getCurrentUsername(),
                            LogUtils.getRemoteIpClient(), LogUtils.getUrl(), ParamNodeValController.class.getName(),
                            Thread.currentThread().getStackTrace()[1].getMethodName(),
                            (obj.getCommandDetailId() != null ? LogUtils.ActionType.UPDATE : LogUtils.ActionType.CREATE),
                            cmdDetail.toString(), LogUtils.getRequestSessionId());
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
                }
                //20180620_tudn_end ghi log DB
                ...
                paramInputService.saveOrUpdate(params);//20180620_tudn_start ghi log DB
                try {
                    LogUtils.logAction(LogUtils.appCode, startTime, new Date(), SessionUtil.getCurrentUsername(),
                            LogUtils.getRemoteIpClient(), LogUtils.getUrl(), ParamNodeValController.class.getName(),
                            Thread.currentThread().getStackTrace()[1].getMethodName(),
            ...
            try {
                LogUtils.logAction(LogUtils.appCode, startTime, new Date(), SessionUtil.getCurrentUsername(),
                        LogUtils.getRemoteIpClient(), LogUtils.getUrl(), CommandController.class.getName(),
                        Thread.currentThread().getStackTrace()[1].getMethodName(),
                        (isEdit ? LogUtils.ActionType.UPDATE : LogUtils.ActionType.CREATE),
    ...
    public StreamedContent onDownloadTemplate() {
        ...
        try {
            ...
            String templatePath = context.getRealPath("/") + "templates" + File.separator
                    + "import" + File.separator + (isShowNodeType ? (locate.equals("vi") ? "ImportCmdTemplate_vi.xlsx" : "ImportCmdTemplate_en.xlsx") : (locate.equals("vi") ? "ImportCmdTemplate_noNodeType_vi.xlsx" : "ImportCmdTemplate_noNodeType_en.xlsx"));
            /*20180705_hoangnd_fix bug da ngon ngu_end*/

            wb = WorkbookFactory.create(new File(templatePath));
            Sheet sheet = wb.getSheetAt(0);

            /*20180709_hoangnd_fix bug da ngon ngu_start*/
            Sheet hiddenSheet = wb.getSheet("hidden");
            if(hiddenSheet == null)
                hiddenSheet = wb.createSheet("hidden");
            /*20180709_hoangnd_fix bug da ngon ngu_end*/
...
//            String fileTemplate = CommonExport.getTemplateExport();
            ...
            locate = languageBean.getLocaleCode();
            if(locate.equals("en")) {
                fileTemplate = ctx.getRealPath("/")
                        + File.separator + "templates" + File.separator + "TEMPLATE_EXPORT_en.xlsx";
            } else {
                fileTemplate = ctx.getRealPath("/")
                        + File.separator + "templates" + File.separator + "TEMPLATE_EXPORT_vi.xlsx";
            }
            /*20180709_hoangnd_fix bug da ngon ngu_end*/
        ...
        try {
            for (ParamInput paramInput : params) {
                ...
                if (paramInput.getParamValue() != null && !paramInput.getParamValue().trim().isEmpty()) {
                    value = StringEscapeUtils.escapeHtml(paramInput.getParamValue());
                }
...
//            }
            ...
            // check cmdSend co phai blacklist hay khong
            Map<String, Object> filters = new HashMap<>();
            filters.put("vendor.vendorId", commandSend.getNodeRun().getVendor().getVendorId());
            filters.put("version.versionId", commandSend.getNodeRun().getVersion().getVersionId());
            filters.put("nodeType.typeId", commandSend.getNodeRun().getNodeType().getTypeId());
            ...
            if (cmdBlacklists != null && !cmdBlacklists.isEmpty()) {
                ...
                if (cmdBlController.checkCommandBlacklist(commandSend.getCmdSend(), cmdBlacklists)){
                    MessageUtil.setErrorMessage(MessageUtil.getResourceBundleMessage("error.isCmdBlacklist"));
//                    logAction = com.viettel.util.LogUtils.addContent(logAction, "Result: " + MessageUtil.getResourceBundleMessage("error.isCmdBlacklist"));
                    isValid = false;
                }
            }
            ...
            switch (commandSend.getCommandDetail().getProtocol().toUpperCase()) {
                case Config.PROTOCOL_TELNET:
                    //20181107_tudn_start check command da quoc gia
...
//                            commandSend.getNodeRun().getNodeType().getTypeName());
                    ...
                    CheckCommandObject checkCommandObject = new CheckCommandObject();
                    checkCommandObject.setNodeId(commandSend.getNodeRun().getNodeId());
                    checkCommandObject.setProtocol(commandSend.getCommandDetail().getProtocol().toUpperCase());
                    checkCommandObject.setUsername(commandSend.getAccount());
                    checkCommandObject.setPass(PassProtector.encrypt(commandSend.getPassword(), Config.SALT));
                    checkCommandObject.setIp(commandSend.getNodeRun().getEffectIp());
                    checkCommandObject.setPort(commandSend.getNodeRun().getPort() == null ? 23 : commandSend.getNodeRun().getPort());
                    checkCommandObject.setPrompt(prompt);
                    checkCommandObject.setCmdExit("exit");
                    checkCommandObject.setTerminator("VT100");
                    checkCommandObject.setCommand(commandSend.getCmdSend());
                    checkCommandObject.setTimeout(timeout);
                    checkCommandObject.setCmdEnd(commandSend.getCommandDetail().getCommandTelnetParser().getCmdEnd());
                    checkCommandObject.setPagingCmd(commandSend.getCommandDetail().getCommandTelnetParser().getPagingCmd());
                    checkCommandObject.setPagingRegex(commandSend.getCommandDetail().getCommandTelnetParser().getPagingRegex());
                    checkCommandObject.setConfirmCmd(commandSend.getCommandDetail().getCommandTelnetParser().getConfirmCmd());
                    checkCommandObject.setConfirmRegex(commandSend.getCommandDetail().getCommandTelnetParser().getConfirmRegex());
                    checkCommandObject.setVendor(commandSend.getNodeRun().getVendor().getVendorName());
                    checkCommandObject.setOsType(commandSend.getNodeRun().getOsType());
                    checkCommandObject.setVersion(commandSend.getNodeRun().getVersion().getVersionName());
                    checkCommandObject.setNodeType(commandSend.getNodeRun().getNodeType().getTypeName());
                    checkCommandObject.setNodeCode(commandSend.getNodeRun().getNodeCode());
                    checkCommandObject.setNodeIp(commandSend.getNodeRun().getNodeIp());
                    checkCommandObject.setCountryCode(commandSend.getNodeRun().getCountryCode().getCountryCode());
                    checkCommandObject.setServerId(commandSend.getNodeRun().getServerId().toString());
                    checkCommandObject.setUserTD(SessionUtil.getCurrentUsername());

                    String encrytedCheckCmd = new String(org.apache.commons.codec.binary.Base64.encodeBase64(("CheckCommand:" + (new Gson()).toJson(checkCommandObject)).getBytes("UTF-8")), "UTF-8");
                    logger.info("vao tien trinh check command");
                    String socketResult = checkCmdThreadExecute(encrytedCheckCmd, commandSend.getNodeRun().getCountryCode().getCountryCode());
                    if (socketResult == null) {
                        commandSend.setResultDetail("Not found process of nation");
                        paramValue.add("NOK");
//                        commandSend.setResultFinal("FALSE");
                        commandSend.setResult("NOK");
                        logger.info("Not found process of nation");
                        return;
                    }
                    //20180814_Quytv7_them loai loi check command start
                    if(socketResult.startsWith("Error:")){
                        commandSend.setResultDetail(socketResult);
                        paramValue.add("NOK");
//                        commandSend.setResultFinal("FALSE");
                        commandSend.setResult("NOK");
                        return;
                    }
                    //20180814_Quytv7_them loai loi check command end
                    Gson gson = (new GsonBuilder()).create();
                    resultDetail = gson.fromJson(socketResult, String.class);
                    //20181107_tudn_start check command da quoc gia
...
//                            commandSend.getCommandDetail().getCommandTelnetParser().getPagingCmd());
                    ...
                    checkCommandObject = new CheckCommandObject();
                    checkCommandObject.setNodeId(commandSend.getNodeRun().getNodeId());
                    checkCommandObject.setProtocol(commandSend.getCommandDetail().getProtocol().toUpperCase());
                    checkCommandObject.setUsername(commandSend.getAccount());
                    checkCommandObject.setPass(PassProtector.encrypt(commandSend.getPassword(), Config.SALT));
                    checkCommandObject.setIp(commandSend.getNodeRun().getEffectIp());
                    checkCommandObject.setPort(22);
                    checkCommandObject.setCommand(commandSend.getCmdSend());
                    checkCommandObject.setTimeout(timeout);
                    checkCommandObject.setPrompt(prompt);
                    checkCommandObject.setPagingCmd(commandSend.getCommandDetail().getCommandTelnetParser().getPagingCmd());
                    ...
                    encrytedCheckCmd = new String(org.apache.commons.codec.binary.Base64.encodeBase64(("CheckCommand:" + (new Gson()).toJson(checkCommandObject)).getBytes("UTF-8")), "UTF-8");
                    logger.info("vao tien trinh check command");
                    socketResult = checkCmdThreadExecute(encrytedCheckCmd, commandSend.getNodeRun().getCountryCode().getCountryCode());
                    if (socketResult == null) {
                        commandSend.setResultDetail("Not found process of nation");
                        paramValue.add("NOK");
//                        commandSend.setResultFinal("FALSE");
...
//                        commandSend.setResultFinal("FALSE");
                    ...
                    resultDetail = gson.fromJson(socketResult, String.class);
                    //20181107_tudn_end check command da quoc gia
                ...
                case Config.PROTOCOL_SQL:
                    try {
                        ...
                        checkCommandObject.setPass(PassProtector.encrypt(commandSend.getPassword(), Config.SALT));
                        checkCommandObject.setCommand(commandSend.getCmdSend());
                        checkCommandObject.setColumn(commandSend.getCommandDetail().getCommandTelnetParser().getColumnOutput() == null
                                ? 0 : commandSend.getCommandDetail().getCommandTelnetParser().getColumnOutput().intValue());
                        checkCommandObject.setJdbcUrl(commandSend.getNodeRun().getJdbcUrl());
                        encrytedCheckCmd = new String(org.apache.commons.codec.binary.Base64.encodeBase64(("CheckCommand:" + (new Gson()).toJson(checkCommandObject)).getBytes("UTF-8")), "UTF-8");
                        logger.info("vao tien trinh check command");
                        socketResult = checkCmdThreadExecute(encrytedCheckCmd, commandSend.getNodeRun().getCountryCode().getCountryCode());
                        if (socketResult == null) {
                        ...
                        //20180814_Quytv7_them loai loi check command end
                        gson = (new GsonBuilder()).create();
                        paramValue = Arrays.asList(gson.fromJson(socketResult, String[].class));
...
//                                , params);
    ...
    //20181107_tudn_start_Check command da quoc gia
    private String checkCmdThreadExecute(String encrytedCheckCmd, String countryCode) throws Exception {
        Map<String, Object> filters = new HashMap<>();
        ...
        List<MapProcessCountry> maps = new MapProcessCountryServiceImpl().findList(filters, null);
...
//                    serverIp = "localhost";
                    ...
                    client.sendMsg(encrytedCheckCmd);
